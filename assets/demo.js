(() => {
  ...
  function setProgress(index) {
    progress.forEach((el, i) => el.classList.toggle('active', i === index));
  }
    ...
    return `
      <div class="note-page">
        <div class="note-toolbar ${step >= 1 ? 'visible' : ''}">
          <button><strong>B</strong></button><button>H1</button><button>H2</button><button>• List</button><button>☑ To-do</button><button>↗ Link</button>
        </div>
  ...
  function promptsHtml(order = ['Launch summary', 'Turn notes into actions', 'Find open questions'], highlight = '') {
    return `<div class="prompt-list">${order.map((name, i) => `
      <div class="prompt-card ${highlight === name ? 'dragging-card' : ''}" data-prompt="${name}">
        <div class="drag-handle">⋮⋮</div><div><strong>${name}</strong><span>${i === 0 ? 'Summarize this conversation into decisions and next steps.' : i === 1 ? 'Turn these notes into a short action list.' : 'List the unresolved questions in this chat.'}</span></div><button class="send-prompt">Send to ChatGPT</button>
      </div>`).join('')}</div>`;
  }
  ...
  function dashboardHtml() {
    return `<div class="dashboard-demo"><div class="dash-filters"><button class="active">All</button><button>High priority</button><button>Product launch</button></div>
      <button class="dash-row high" data-chat="research"><span class="dot high"></span><div><strong>User interview synthesis</strong><span>High priority · Follow-up</span></div></button>
      ...
      <button class="dash-row" data-chat="launch"><span class="dot low"></span><div><strong>Website launch plan</strong><span>Low priority · Open</span></div></button>
    </div>`;
  }

  function searchHtml(query = '') {
    return `<div class="search-demo"><div class="search-box"><span>⌕</span><span class="query">${query || 'Search notes, prompts and conversations...'}</span></div>
      ${query ? `<div class="search-results"><button data-chat="seo"><small>CONVERSATION</small><strong>SEO content plan</strong><span>Questions people ask about ChatGPT workflow...</span></button><button><small>NOTE</small><strong>Launch decisions</strong><span>Publish local-first trust messaging...</span></button><button><small>PROMPT</small><strong>Find open questions</strong><span>List the unresolved questions in this chat.</span></button></div>` : ''}</div>`;
  }
  ...
  function runNotes() {
    ...
    later(() => {
      const p = assistantMsg;
      p.classList.add('selected-text');
      setCursor(260, 245, false, true);
    }, 900);
    later(() => {
      panelBody.innerHTML = notesHtml(1);
      assistantMsg.classList.remove('selected-text');
      setCursor(715, 276, true);
      clickPulse();
    }, 2200);
    later(() => {
      panelBody.innerHTML = notesHtml(2);
      setCursor(695, 205, true);
      clickPulse();
    }, 3600);
    later(() => {
      panelBody.innerHTML = notesHtml(3);
      setCursor(760, 205, true);
      clickPulse();
    }, 4800);
    later(() => {
      panelBody.innerHTML = notesHtml(4);
      setCursor(812, 205, true);
      clickPulse();
    }, 6000);
    later(() => {
      const add = demo.querySelector('.add-subpage');
      add.classList.add('pulse-control');
      setCursor(902, 171, true);
      clickPulse();
      later(() => {
        add.classList.remove('pulse-control');
        const btn = demo.querySelector('.subpage-btn');
        btn.textContent = 'Launch checklist';
        btn.classList.add('active');
        demo.querySelector('.cm-subtabs > button:first-child').classList.remove('active');
      }, 400);
    }, 7300);
    later(() => {
      setCursor(880, 58, true);
      clickPulse();
      status.textContent = 'Workspace: open the same note in a larger writing surface.';
      showWorkspace();
    }, 8800);
    later(() => {
      showChat();
  ...
  function runPrompts() {
    ...
    later(() => {
      ...
      setCursor(692, 245, true);
      clickPulse();
    }, 4200);
    later(() => {
      const first = panelBody.querySelector('.prompt-card');
      first.classList.toggle('collapsed');
    }, 5400);
    later(() => runDashboard(), 6800);
  }
  ...
  function runDashboard() {
    ...
    setCursor(812, 126, true);
    clickPulse();

    later(() => {
      const high = panelBody.querySelectorAll('.dash-filters button')[1];
      high.classList.add('active');
      panelBody.querySelectorAll('.dash-filters button')[0].classList.remove('active');
      setCursor(760, 205, true);
      clickPulse();
    }, 1500);
    later(() => {
      setCursor(805, 276, true);
      clickPulse();
      setChat('research');
    }, 2900);
    later(() => {
      setCursor(808, 332, true);
      clickPulse();
      setChat('seo');
    }, 4500);
    later(() => runSearch(), 6100);
  }
  ...
  function runSearch() {
    ...
    setCursor(912, 126, true);
    clickPulse();

    later(() => {
      panelBody.innerHTML = searchHtml('local-first');
      setCursor(760, 206, true);
      clickPulse();
    }, 1400);
    later(() => {
      const result = panelBody.querySelector('.search-results button');
      result.classList.add('selected-result');
      setCursor(780, 277, true);
      clickPulse();
      setChat('seo');
    }, 3000);
    later(() => {
  ...
  demo.querySelectorAll('[data-open-workspace]').forEach(btn => btn.addEventListener('click', () => {
    clearTimers();
    running = false;
    showWorkspace();
    status.textContent = 'Workspace: the same note, opened in a larger writing surface.';
  }));
  chromeTabs.forEach(tab => tab.addEventListener('click', () => {
    clearTimers();
    running = false;
    if (tab.dataset.browserTab === 'workspace') showWorkspace(); else showChat();
  }));
